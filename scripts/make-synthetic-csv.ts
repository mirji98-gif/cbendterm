 ...
 * v4 group-code assignment scheme (change_spec_v4_final.md), with a
 ...
 *
 * v4 drops the recruiter dimension (Part 1: three links, one per arm) and
 * gives each brand two pop-ups instead of one (Part 2), so this generates a
 * p1 AND a p2 PopupResult per block. Two rows use assignment_source='random'
 * to exercise that path (a mistyped or missing code), plus the usual
 * dropout/debug rows. The simulated effect follows the PRD's own predictions:
 * strong shame raises irritation and perceived manipulation most, mild sits
 * in between, autonomy is close to neutral. Comparative and awareness answers
 * are simulated with the SAME recode functions the app uses. Do not read
 * anything into the numbers — invented.
 */
...
import {
  blockAtPosition, brandForBlock, emptyPopupResult,
  type AssignmentSource, type BlockData, type BlockKey, type PopupResult, type Session,
...
// 15 per arm (change_spec_v4_final.md Part 1: three links, no recruiter
...
/** Simulated latency inflation (ms) on the experimental pop-ups, by arm. */
...
/** Probability of accepting a pop-up (both p1 and p2 draw independently), by condition. */
const ACCEPT_PROB: Record<'neutral' | Arm, number> = {
  neutral: 0.24, mild: 0.19, strong: 0.13, autonomy: 0.24,
};

function pickChoice(condition: 'neutral' | Arm): Choice {
  const r = rand();
  const accept = ACCEPT_PROB[condition];
  if (r < accept) return 'accept';
  const r2 = (r - accept) / (1 - accept);
  if (condition === 'neutral') return r2 < 0.6 ? 'decline_button' : r2 < 0.94 ? 'close_x' : 'backdrop';
  if (condition === 'strong') return r2 < 0.45 ? 'decline_button' : r2 < 0.86 ? 'close_x' : r2 < 0.97 ? 'backdrop' : 'timeout';
  if (condition === 'mild') return r2 < 0.55 ? 'decline_button' : r2 < 0.92 ? 'close_x' : 'backdrop';
  return r2 < 0.68 ? 'decline_button' : r2 < 0.94 ? 'close_x' : 'backdrop';
}
...
function makePopup(condition: 'neutral' | Arm, latencyBase: number): PopupResult {
  const choice = pickChoice(condition);
  const latency = Math.max(300, gauss(latencyBase, 900));
  return {
    choice,
    latencyMs: Math.round(latency * 10) / 10,
    timeToFirstTouchMs: Math.round(Math.max(150, latency * 0.45) * 10) / 10,
    cancelledTaps: rand() < (condition === 'neutral' ? 0.12 : condition === 'strong' ? 0.34 : 0.2) ? 1 : 0,
    pointerCancels: Math.floor(rand() * 4),
    pressDwellMs: Math.round(gauss(140, 45)),
    postDismissDwellMs: Math.round(Math.max(80, gauss(1200, 900))),
    continuationAutoAdvanced: rand() < 0.15,
    scrollEvents: Math.floor(rand() * 18) + 2,
    rageTaps: rand() < (condition === 'strong' ? 0.14 : 0.04) ? 1 : 0,
...
function makeBlock(
  ...
  const latencyBase = 2100 + (key === 'neutral' ? 0 : LATENCY_SHIFT[arm]);
  ...
  return {
    ...
    p1: makePopup(condition, latencyBase),
    p2: makePopup(condition, latencyBase * 0.9),
    ratings,
...
draws.forEach((arm, i) => {
  ...
  // assignment_source='random', arm still drawn (uniformly, independent of
  // the arm this loop assigned — a random-fallback participant's arm isn't
  // "this loop's arm" in the real app either).
